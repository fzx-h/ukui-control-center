    ...
    : QWidget(parent)
{
    ...
    m_pAppWidget->m_pAppArea = new QScrollArea(m_pAppWidget);
    m_pAppWidget->m_pDisplayAppVolumeWidget = new QWidget(m_pAppWidget->m_pAppArea);
    m_pAppWidget->m_pAppArea->setWidget(m_pAppWidget->m_pDisplayAppVolumeWidget);
    m_pAppWidget->m_pVLayout = new QVBoxLayout(m_pAppWidget->m_pDisplayAppVolumeWidget);

    m_pAppWidget->m_pAppArea->setMaximumSize(550,150);
    m_pAppWidget->m_pAppArea->setMaximumSize(960,150);
    m_pAppWidget->m_pDisplayAppVolumeWidget->setMinimumWidth(550);
    m_pAppWidget->m_pDisplayAppVolumeWidget->setMaximumWidth(960);
...
//    qDebug() << "on stream control removed" << m_pName;
...
//    qDebug() << "remove application control" <<m_pName << "indexx :" << i;
    if (i < 0)
        return;
...
//    }
    ...
    qDebug() << "*************count" << m_pWidget->m_pAppWidget->m_pVLayout->count() << i;
    m_pWidget->m_pStreamControlList->removeAt(i);
    m_pWidget->m_pAppNameList->removeAt(i);
    //当播放音乐的应用程序退出后删除该项
    QLayoutItem *item ;
    if ((item = m_pWidget->m_pAppWidget->m_pVLayout->takeAt(i)) != 0) {
        item->widget()->setParent(nullptr);
        delete item;
    }
    m_pWidget->m_pAppWidget->m_pVLayout->update();
    m_pWidget->m_pAppWidget->m_pVLayout->setContentsMargins(0,0,0,0);
    m_pWidget->m_pAppWidget->m_pVLayout->widget()->adjustSize();
    if (appnum <= 0) {
        g_warn_if_reached ();
        appnum = 1;
    }
    qDebug() << "删除后count" << m_pWidget->m_pAppWidget->m_pVLayout->count();
    appnum--;
    if (appnum <= 0)
...
void UkmediaMainWidget::addAppToAppwidget(UkmediaMainWidget *m_pWidget,int appnum, const gchar *m_pAppName,QString appIconName,MateMixerStreamControl *m_pControl)
{
    ...
    //widget显示应用音量
    m_pWidget->m_pApplicationWidget = new QWidget(m_pWidget->m_pAppWidget->m_pDisplayAppVolumeWidget);
...
//    m_pWidget->appIconBtn->setEnabled(true);
    ...
    qDebug() << "appslider name :" << m_pAppName <<" app name 总数：" << m_pWidget->m_pAppNameList->count();
...
//        slider_str.append("Slider");
    ...
    m_pWidget->m_pAppWidget->m_pVLayout->addWidget(m_pWidget->m_pApplicationWidget);
    m_pWidget->m_pAppWidget->m_pVLayout->setSpacing(1);
    m_pWidget->m_pAppWidget->m_pVLayout->setContentsMargins(0,0,0,0);//m_pWidget->m_pAppWidget->height()-appnum*52);
//    m_pWidget->m_pAppWidget->m_pVLayout->widget()->adjustSize();
...
//    qDebug() << "index :" << index << "stream control list size:" << m_pWidget->m_pStreamControlList->count() << "app name size:" << m_pWidget->m_pAppVolumeList->count();
